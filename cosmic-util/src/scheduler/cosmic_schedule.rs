...
impl CosmicSchedule {
    ...
    pub fn parse(crontab_schedule: &str) -> Result<CosmicSchedule, CrontabError> {
        let schedule = parse_cron(crontab_schedule)?;
        Ok(CosmicSchedule { schedule: schedule })
    }

    pub fn next_event(&self, start_time: &CosmicTime) -> Option<CosmicTime> {
...
enum DateTimeMatch {
    Missed,
    ContinueMatching,
    AnswerFound(CosmicTime),
}

fn try_month(times: &ScheduleComponents, time: &mut CosmicTime) -> DateTimeMatch {
...
fn try_day(times: &ScheduleComponents, time: &mut CosmicTime) -> DateTimeMatch {
...
fn try_weekday(times: &ScheduleComponents, time: &mut CosmicTime) -> DateTimeMatch {
...
fn try_hour(times: &ScheduleComponents, time: &mut CosmicTime) -> DateTimeMatch {
...
fn try_minute(times: &ScheduleComponents, time: &mut CosmicTime) -> DateTimeMatch {
...
mod tests {
    ...
    fn test_first_of_the_month() {
        let cron = CosmicSchedule::parse("0 0 1 * *").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40");
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
    ...
    #[test]
    fn test_feb_28th() {
        let cron = CosmicSchedule::parse("15 7 28 2 *").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40");
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
    ...
    #[test]
    fn test_feb_29th() {
        let cron = CosmicSchedule::parse("15 7 29 2 *").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40");
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
    ...
    fn test_out_day_of_month_range() {
        let cron = CosmicSchedule::parse("* * 30,31 2 *").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40");
        assert!(cron.next_event(&CosmicTime::from_time_ts(from_ts)).is_none());

        let cron = CosmicSchedule::parse("* * 31 4,6,9,11 *").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40");
        assert!(cron.next_event(&CosmicTime::from_time_ts(from_ts)).is_none());
    }
    ...
    #[test]
    fn test_next_year() {
        let cron = CosmicSchedule::parse("* * * 1 *").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40");
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
    ...
    #[test]
    fn test_next_month() {
        let cron = CosmicSchedule::parse("* * * 12 *").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40");
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
        assert_eq!(
            to_datetime(next_execution.to_time_ts(0).unwrap()),
        ...
        let cron = CosmicSchedule::parse("* * 25 * *").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40");
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
        assert_eq!(
            to_datetime(next_execution.to_time_ts(0).unwrap()),
    ...
    #[test]
    fn test_next_day() {
        let cron = CosmicSchedule::parse("55 2 30 * *").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40");
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
        assert_eq!(
            to_datetime(next_execution.to_time_ts(0).unwrap()),
        ...
        let cron = CosmicSchedule::parse("1 1 * * *").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40");
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
        assert_eq!(
            to_datetime(next_execution.to_time_ts(0).unwrap()),
    ...
    #[test]
    fn test_next_hour() {
        let cron = CosmicSchedule::parse("* 2 * * *").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40");
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
        assert_eq!(
            to_datetime(next_execution.to_time_ts(0).unwrap()),
        ...
        let cron = CosmicSchedule::parse("15 * * * *").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40");
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
        assert_eq!(
            to_datetime(next_execution.to_time_ts(0).unwrap()),
    ...
    fn test_next_minutes() {
        let cron = CosmicSchedule::parse("21 * * * *").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40");
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
        assert_eq!(
            to_datetime(next_execution.to_time_ts(0).unwrap()),
        ...
        let cron = CosmicSchedule::parse("*/5 * * * *").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40");
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
        assert_eq!(
            to_datetime(next_execution.to_time_ts(0).unwrap()),
    ...
    fn test_next_weekday() {
        ...
        // Find next Saturday
        let cron = CosmicSchedule::parse("* * * * 6").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40"); // Monday
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
        assert_eq!(
            to_datetime(next_execution.to_time_ts(0).unwrap()),
        ...
        // Find next Monday
        let cron = CosmicSchedule::parse("0 1 * * 1").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40"); // Monday
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
        assert_eq!(
            to_datetime(next_execution.to_time_ts(0).unwrap()),
        ...
        let cron = CosmicSchedule::parse("0 8 29 2 5").unwrap();
        let from_ts = to_timestamp("2021-11-29 01:16:40"); // Monday
        let next_execution = cron.next_event(&CosmicTime::from_time_ts(from_ts)).unwrap();
        assert_eq!(
            to_datetime(next_execution.to_time_ts(0).unwrap()),
