...
pub(crate) fn adv_day(time: &mut CosmicTime) {
...
pub(crate) fn adv_hour(time: &mut CosmicTime) {
    time.tm_hour += 1;
    if time.tm_hour > 23 {
        time.tm_hour = 0;
        adv_day(time);
...
pub(crate) fn adv_minute(time: &mut CosmicTime) {
    time.tm_min += 1;
    if time.tm_min > 59 {
        time.tm_min = 0;
        adv_hour(time);
...
mod tests {
    ...
    pub(crate) fn normal(time: &CosmicTime) -> CosmicTime {
    ...
    pub fn test_adv_year() {
        let mut tm = CosmicTime::get_tm(2017, 10, 6, 12, 24, 0);
        adv_year(&mut tm);
        assert_eq!(normal(&tm), CosmicTime::get_tm(2018, 10, 6, 12, 24, 0));
    }

    #[test]
    pub fn test_adv_month() {
        // January
        let mut tm = CosmicTime::get_tm(2017, 1, 1, 12, 0, 0);
        adv_month(&mut tm);
        assert_eq!(normal(&tm), CosmicTime::get_tm(2017, 2, 1, 12, 0, 0));

        // December
        let mut tm = CosmicTime::get_tm(2017, 12, 1, 0, 0, 0);
        adv_month(&mut tm);
        assert_eq!(normal(&tm), CosmicTime::get_tm(2018, 1, 1, 0, 0, 0));
    }

    #[test]
    pub fn test_adv_day_on_mday() {
        // 2017-01-01 00:00 UTC, a non-leap year starting on a Sunday (tm_wday=0).
        let mut tm = CosmicTime::from_time_ts(1483228800);
    ...
    pub fn test_adv_day_on_wday() {
        ...
        let mut tm = CosmicTime::from_time_ts(sunday);
